...
public class UserPlayedLottoAndWonIntegrationTest extends BaseIntegrationTest {
    ...
    public void should_user_win_and_system_should_generate_winners() throws Exception {
        ...
        String jason= mvcResult.getResponse().getContentAsString();
        NumberReceiverResponseDto numberReceiverResponseDto = objectMapper.readValue(jason, NumberReceiverResponseDto.class);
        ...
        //give
        //when
        ResultActions performGetResultsWithNoExistingId = mockMvc.perform(get("/results/" + "notExistingId"));

        //then
        performGetResultsWithNoExistingId.andExpect(status().isNotFound())
                .andExpect(content().json("""
                        {
                        "message": "Not found from id: notExistingId",
                        "status": "NOT_FOUND"
                        }""".trim()
                ));
        ...
        //give
        //when
        ResultActions performGetResultsWithExistingId = mockMvc.perform(get("/results/" + ticketId));

        //then
        MvcResult mvcResultGetMethod = performGetResultsWithExistingId.andExpect(status().isOk()).andReturn();
        String jonGetMethod = mvcResultGetMethod.getResponse().getContentAsString();
        ResultAnnouncerResponseDto finalResult = objectMapper.readValue(jonGetMethod, ResultAnnouncerResponseDto.class);
        assertAll(
